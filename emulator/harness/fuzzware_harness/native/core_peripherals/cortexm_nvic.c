...
static int get_boosted_prio(int raw_prio) {
    // if GET_PRIMASK() equals 1, it means the IRQ should be masked.
    if(GET_PRIMASK()
...
// Accessing the SCS region, including NVIC, systick, etc.
...
void PushStack(uc_engine *uc, bool skip_instruction) {
    ...
    // Reading r0, r1, r2, r3, r12, lr, pc_retaddr, xpsr_retspr, sp;
...
static void ExceptionEntry(uc_engine *uc, bool is_tail_chained, bool skip_instruction) {
    ...
    // xpsr[5:0] is the exception number

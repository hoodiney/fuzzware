...
class LivenessExtendPlugin(angr.SimStatePlugin):
    base_snapshot: BaseStateSnapshotExtend
    ...
    def __init__(self, base_snapshot, alive_vars=None, tracked_vars=None, returned=False, stackframes=None, all_vars_dead=False):
        super(LivenessExtendPlugin, self).__init__()
    ...
    def copy(self, memo):
        return LivenessExtendPlugin(self.base_snapshot, copy.deepcopy(self.alive_varname_counts), copy.deepcopy(self.tracked_vars), returned=self.returned, stackframes=[f.copy() for f in self.stackframes], all_vars_dead=self.all_vars_dead)

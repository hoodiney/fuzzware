...
from .arch_specific.arm_regs import STATE_SNAPSHOT_REG_LIST, translate_reg_name_to_vex_internal_name, leave_reg_untainted, REG_NAME_PC, REG_NAME_SP
...
class BaseStateSnapshotExtend:
    ...
    def from_state_file(cls, statefile, cfg):
        ...
        initial_state = project.factory.blank_state(addr=entry_addr)

        arm_thumb_quirks.add_special_initstate_reg_vals_extend(initial_state, regs)

        # apply registers to state
        initial_sp = None
        for name, val in regs.items():
            if name == REG_NAME_PC:
                base_snapshot.initial_pc = entry_addr
                continue
